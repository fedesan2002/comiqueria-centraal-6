...
// reduce: acumula el precio de cada producto guardado en el carrito.
...
function aplicarFiltroCategoria() {
    const selector = document.getElementById("categoriaFiltro");
    const mensaje = document.getElementById("mensajeBusqueda");

    categoriaSeleccionada = selector.value;

    const productosFiltrados = filtrarProductosPorCategoria(
    ...
    if (categoriaSeleccionada === "Todos") {
        mensaje.textContent =
            "Mostrando los " + productosFiltrados.length +
            " productos del catálogo.";
    } else {
        mensaje.textContent =
            "Mostrando " + productosFiltrados.length +
            " producto(s) de la categoría " + categoriaSeleccionada + ".";
    }
...
function iniciarSimulador() {
    const productosDisponibles = obtenerCatalogoVisible();
    ...
    const resultado = buscarProducto(productoIngresado, productosDisponibles);

    if (!resultado.existe) {
        alert(
            productoIngresado +
            " no se encuentra en el catálogo seleccionado."
        );
        return;
    }
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document
        ...
        .addEventListener("change", aplicarFiltroCategoria);
